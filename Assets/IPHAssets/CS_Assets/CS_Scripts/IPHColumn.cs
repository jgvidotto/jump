...
namespace InfiniteHopper
{
	...
	public class IPHColumn:MonoBehaviour 
	{
        ...
        //Add the time the player takes to jump to the next column, predicting where it's going to be by the time he lands
        public float Predicted(float time)
        {
            return moveRange.x + (moveRange.y - moveRange.x) / 2 + Mathf.Sin(moveSpeed * Time.time + startingHeight + time) * ((moveRange.y - moveRange.x) / 2);
        }
