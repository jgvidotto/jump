...
namespace InfiniteHopper
{
	...
	public class IPHGameController: MonoBehaviour
    {
        ...
        internal int totalTokens = 0;
        internal int currentPowerUpStreak = 0;
		internal int longestPowerUpStreak = 0;
        internal int numberOfMatches = 0;
        public List<Transform> vectorsColumn = new List<Transform>();
        public List<Transform> vectorsItens = new List<Transform>();

        public PlayerStats playerStats;

        //Pass the id of the achievement and the current score to its progression field.
        public delegate void OnVariableChangeDelegate(int key, int newVal);
        public event OnVariableChangeDelegate OnVariableChange;

        public delegate void SaveTransform(SerializableTransform serializable, bool sameScene);
        public event SaveTransform OnSaveTransform;

		public delegate void ShouldSaveScene(bool addScene);
		public event ShouldSaveScene OnSceneEnter;
        ...
        void Start()
		{
            ...
            if (PlayerPrefs.GetInt("playagain") == 1) createLoadedColumn = true;
            else createLoadedColumn = false;
            ...
            UpdateScore ();
                ...
                CreateLoadedColumn(precreateColumns, true, createLoadedColumn);
		...
		void  Update()
		{
			...
			{
                CreateLoadedColumn(1, true, createLoadedColumn);
			}
		}


        void CreateLoadedColumn(int columnCount, bool giveBonus, bool start)
        {
            OnVariableChange?.Invoke(8, playerStats.numberOfMatches);

            OnVariableChange?.Invoke(9, playerStats.numberOfMatches);

            var sceneCount = GetComponent<LoadSceneData>().Data.scenes.Count;
            while (columnCount > 0)
            {
                columnCount--;

                if (sceneCount > 0 && start)
                {

                    var transforms = GetComponent<LoadSceneData>().Data.scenes.Single(idS => idS.id == PlayerPrefs.GetString("id"));

                    var listPositions = transforms.transforms;
                    List<SerializableTransform> transformList = listPositions.ToList();

                    InstantiateColumn(transformList);
                }
                else
                {
                    createColumn(giveBonus, false);
                }
            }

        }

        public void InstantiateColumn(List<SerializableTransform> transforms)
        {
            
            Transform newColumn;
            Vector3 columnPosition;
            Transform newItem;
            int randomColumn = Mathf.FloorToInt(Random.Range(0, movingColumns.Length));

            if(transforms.Count -1 > currentColumn + 1)
            {
                columnPosition = new Vector3(transforms[currentColumn].column.x, transforms[currentColumn].column.y, transforms[currentColumn].column.z);
                if (transforms[currentColumn].movingColumn)
                {

                    newColumn = Instantiate(movingColumns[randomColumn], columnPosition, Quaternion.identity) as Transform;
                }
                else
                {
                    newColumn = Instantiate(columns[randomColumn], columnPosition, Quaternion.identity) as Transform;
                    if (currentColumn == 0) newColumn.SendMessage("NoBonus");
                }
                if (transforms[currentColumn].item != null)
                {
                    newItem = Instantiate(items[Mathf.FloorToInt(Random.Range(0, items.Length))], newColumn.position, Quaternion.identity);
                }

                nextColumnPosition.x = transforms[currentColumn + 1].column.x;
                currentColumn++;
            }
            else
            {
                createColumn(true, true);
                currentColumn++;
            }
        ...
        //Should reset the level
        public void PlayAgainButton()
        {
            PlayerPrefs.SetInt("playagain", 1);
            createLoadedColumn = true;
            Restart();
        }

        //This function creates a column
        void createColumn(bool giveBonus, bool keepScene)
		{
            ...
            vectorsColumn.Add(newColumn);

			// Record the first column we land on
			if ( giveBonus == false )    lastLandedObject = newColumn;
            if(nextColumnPosition.x <= 0f) newColumn.SendMessage("NoBonus");
            ...
            if (startingPowerUp == 1) {
                newItem = Instantiate( items[Mathf.FloorToInt(Random.Range(0, items.Length))], newColumn.position, Quaternion.identity);
                vectorsItens.Add(newItem);
                OnSaveTransform(new SerializableTransform(new Position(newColumn), new Position(newItem), newColumn.GetComponent<IPHColumn>().movingColumn, System.Guid.NewGuid().ToString()), keepScene);
            }
            ...
            {
                OnSaveTransform(new SerializableTransform(new Position(newColumn), null, newColumn.GetComponent<IPHColumn>().movingColumn, System.Guid.NewGuid().ToString()), keepScene);
            }
		...
		void  ChangeScore( Transform landedObject )
		{
			...
			for ( index = 0 ; index < landingBonuses.Length ; index++ )
			{
				...
				if ( bonusGiven == false && landingDistance <= landingBonuses[index].landDistance )    
				{
					...
					if ( index == 0 )    
					{
                        ...
                        OnVariableChange(1, landingBonuses[index].bonusValue * currentStreak * scoreMultiplier);
                        OnVariableChange(2, landingBonuses[index].bonusValue * currentStreak * scoreMultiplier);
                        ...
                        //check if the player has a new streak record and save it
                        if (playerStats.longestStreak < currentStreak)
                        {
            ...
            OnVariableChange(7, score);

            // Add to the longest distance statistic
            currentDistance++;
            OnVariableChange(3, currentDistance);
            OnVariableChange(4, currentDistance);

            longestDistance = currentDistance;

            if(playerStats.longestDistance < longestDistance)
            {
                playerStats.longestDistance = longestDistance;
            }
        }
		...
		IEnumerator GameOver(float delay)
		{
			...
			if ( gameOverCanvas )    
			{
                ...
                if(PlayerPrefs.HasKey("id") && playerStats.highscoreScene[PlayerPrefs.GetString("id")] > highScore)
                {
                    gameOverCanvas.Find("TextHighScore").GetComponent<Text>().text = "HIGHSCORE " + playerStats.highscoreScene[PlayerPrefs.GetString("id")].ToString();
                }
                else
                {
                    
                    gameOverCanvas.Find("TextHighScore").GetComponent<Text>().text = "HIGHSCORE " + highScore.ToString();
                }
        ...
        public void AddScore(Dictionary<string, int> keyValuePairs, string id, int scoreScene)
        {
            if (!keyValuePairs.ContainsKey(id))
            {
                keyValuePairs.Add(id, scoreScene);
            }
            else
            {
                if(score > scoreScene)
                {
                    keyValuePairs[id] = score;
                }
            }
        }
        ...
        public void Save()
        {
            //Check if the score made it to top 10             
            AddScore(playerStats.highscoreScene, PlayerPrefs.GetString("id"), score);
        ...
        public void  Restart ()
		{
            ...
            {
                ChangeBool(false);
                PlayerPrefs.SetString("firstPlay", "first");
            }
            else
            {
                ChangeBool(createLoadedColumn);
            }
        ...
        public void ChangeBool(bool sceneAdd)
        {
            createLoadedColumn = sceneAdd;
            if (!createLoadedColumn)
            {
                PlayerPrefs.DeleteKey("playagain");
                PlayerPrefs.DeleteKey("id");
                Time.timeScale = 1;
            }
			OnSceneEnter(!sceneAdd);
            OnSceneSave(GetComponent<LoadSceneData>().Data);
        }
        ...
        //Add a token when the player grabs a feather
        void TokenCount(int count)
        {
            totalTokens += count;
            tokenText.GetComponent<Text>().text = totalTokens.ToString();
            playerStats.totalTokens += count;

            OnVariableChange(10, totalTokens);
            OnVariableChange(11, totalTokens);
            OnVariableChange(12, playerStats.totalTokens);
            OnVariableChange(13, playerStats.totalTokens);
            OnVariableChange(14, playerStats.totalTokens);
            OnVariableChange(15, playerStats.totalTokens);
            OnVariableChange(16, playerStats.totalTokens);
        ...
        /// </summary>
        /// <param name="powerupIndex">The index numebr of the powerup to activate</param>
        IEnumerator ActivatePowerup( int powerupIndex )
		{
			...
			if ( powerups[powerupIndex].duration > 0 )
			{
                ...
                longestPowerUpStreak = currentPowerUpStreak;

                if(playerStats.longestPowerUp < longestPowerUpStreak)
                {
                    playerStats.longestPowerUp = longestPowerUpStreak;
                }
            }
            ...
            OnVariableChange(5, totalPowerups);
            OnVariableChange(6, totalPowerups);

            if (playerStats.totalPowerUps < totalPowerups)
            {
                playerStats.totalPowerUps = totalPowerups;
            }
		}		
