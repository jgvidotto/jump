  ...
  using System.Collections.Generic;
    ...
    public override void init (string gameId, bool testModeEnabled, string gameObjectName, string unityVersion) {
      ...
      Utils.LogDebug ("UnityEditor: init(), gameId=" + gameId + ", testModeEnabled=" + testModeEnabled + ", gameObjectName=" + gameObjectName);
    	...
    	string url = "https://impact.applifier.com/mobile/campaigns?platform=editor&gameId=" + UnityWebRequest.EscapeURL(gameId) + "&unityVersion=" + UnityWebRequest.EscapeURL(unityVersion);
    	
		AsyncExec.runWithCallback<string, UnityWebRequest> (getAdPlan, url, handleResponse);
  	}

	private IEnumerator getAdPlan(string url, System.Action<UnityWebRequest> callback) {
